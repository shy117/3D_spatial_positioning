...
- `win32`：Windows 构建，使用本机 Qt 6 MinGW 和已有 OpenCV 包，当前默认不启用 PCL/VTK。
...
- 当前 Windows 环境未找到 PCL/VTK，未定义 `HAVE_PCL`，点云按钮会提示“当前构建未启用 PCL/VTK”，但界面、读图、立体匹配、深度图和测距可用。
...
1. Windows 当前构建未启用 PCL/VTK，点云生成和保存暂不可用。
2. Windows 如需完整点云功能，建议准备 ABI 匹配的 MSVC 版本 Qt、OpenCV、PCL、VTK、Boost，再扩展 `.pro` 的 `win32` 分支。
