...
bool Method::savePointCloudAsPly(const QString &filename, const cv::Mat& points_3d, const cv::Mat& colors, QString *errorMessage)
{
    if (points_3d.empty() || colors.empty()) {
        if (errorMessage != nullptr) {
            *errorMessage = "点云坐标或颜色图为空";
        }
        return false;
    }
    ...
    if (points_3d.type() != CV_32FC3) {
        if (errorMessage != nullptr) {
            *errorMessage = "点云坐标矩阵格式不是 CV_32FC3";
        }
        return false;
    }

    if (colors.type() != CV_8UC3 || points_3d.size() != colors.size()) {
        if (errorMessage != nullptr) {
            *errorMessage = "颜色图格式或尺寸与点云坐标不匹配";
        }
        return false;
    }
    ...
    for (int row = 0; row < points_3d.rows; ++row) {
        ...
        for (int col = 0; col < points_3d.cols; ++col) {
            ...
            vertices.push_back({x, y, z, bgr[2], bgr[1], bgr[0]});
        }
    }

    if (vertices.empty()) {
        if (errorMessage != nullptr) {
            *errorMessage = "没有可保存的有效点";
        }
        return false;
    }
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        if (errorMessage != nullptr) {
            *errorMessage = "无法写入点云文件：" + filename;
        }
        return false;
    }
    ...
    stream << "comment generated by 3D_spatial_positioning without PCL\n";
    ...
    for (const PlyPoint &vertex : vertices) {
        stream << vertex.x << ' '
               << vertex.y << ' '
               << vertex.z << ' '
    ...
    if (errorMessage != nullptr) {
        *errorMessage = QString("保存 %1 个点").arg(vertices.size());
    }
    return true;
}
